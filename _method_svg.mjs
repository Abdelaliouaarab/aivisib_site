...
for (const e of ENG) for (const off of [-8, 0, 8]) { const d = cur(qx, qy, e.x, e.y + e.h / 2, off); paths += `<path d="${d}" class="ml"/>`; dots += `<circle r="3.2" cx="-30" cy="-30" class="md"><animateMotion dur="${(2.2 + (k % 5) * 0.25).toFixed(2)}s" begin="${((k * 0.37) % 2.5).toFixed(2)}s" repeatCount="indefinite" path="${d}"/></circle>`; k++; }
for (const e of ENG) { const d = cur(e.x + e.w, e.y + e.h / 2, N.an.x, N.an.y + N.an.h / 2); paths += `<path d="${d}" class="ml"/>`; dots += `<circle r="3.2" cx="-30" cy="-30" class="md"><animateMotion dur="2s" begin="${((k * 0.41) % 2).toFixed(2)}s" repeatCount="indefinite" path="${d}"/></circle>`; k++; }
for (const [p1, p2] of [[N.an, N.st], [N.st, N.rp]]) { const d = `M${p1.x + p1.w},${p1.y + p1.h / 2} L${p2.x},${p2.y + p2.h / 2}`; paths += `<path d="${d}" class="ml thick"/>`; dots += `<circle r="4" cx="-30" cy="-30" class="md"><animateMotion dur="1.6s" repeatCount="indefinite" path="${d}"/></circle>`; }
